...
export default function Toast({ message, type = "success", duration = 5000, onClose }: ToastProps) {
  ...
  return (
    <div className={`fixed top-4 sm:top-8 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm sm:max-w-md px-4 sm:px-0 transition-all duration-300 ${
      ...
      <div className={`flex items-center gap-3 sm:gap-4 bg-white dark:bg-slate-800 rounded-2xl sm:rounded-full px-4 sm:px-8 py-3 sm:py-4 shadow-2xl border border-gray-200 dark:border-slate-700 backdrop-blur-sm ${
        type === "success" 
        ...
        <span className="flex-1 text-sm sm:text-base font-medium text-gray-900 dark:text-gray-100 pr-2">
          {message}
        </span>
        <button
          onClick={handleClose}
